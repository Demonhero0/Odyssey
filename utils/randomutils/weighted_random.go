...
import (
	...
	"github.com/crytic/medusa/fuzzing/utils/container"
...
// Hashable describes an interface for data that can be hashed, which is the fundamental of RemoveHash
type Hashable interface {
	Hash() (common.Hash, error)
}

// isHashable checks if the type T implements the Hashable interface
func isHashable[T any]() bool {
	_, ok := any((*T)(nil)).(Hashable)
	return ok
}
...
type WeightedRandomChooser[T any] struct {
	...
	// hashChoices if T is Hashable, we will use hash linked list instead of choices to speed up the RemoveByHash method.
	hashChoices *container.HashLinkedList[common.Hash, *WeightedRandomChoice[T]]
...
func NewWeightedRandomChooserWithRand[T any](randomProvider *rand.Rand, randomProviderLock *sync.Mutex) *WeightedRandomChooser[T] {
	var choices []*WeightedRandomChoice[T]                                           // nil
	var hashChoices *container.HashLinkedList[common.Hash, *WeightedRandomChoice[T]] // nil

	if isHashable[T]() {
		hashChoices = container.NewHashLinkedList[common.Hash, *WeightedRandomChoice[T]]()
	} else {
...
func (c *WeightedRandomChooser[T]) ChoiceCount() int {
	if c.hashChoices != nil {
		return c.hashChoices.Len()
	} else {
...
func (c *WeightedRandomChooser[T]) AddChoices(choices ...*WeightedRandomChoice[T]) {
	...
	for _, choice := range choices {
		...
		// If we have a hashChoices, we hash the data and add it to our hash linked list.
		if c.hashChoices != nil {
			hash, err := any(choice.Data).(Hashable).Hash()
			if err != nil {
				panic(fmt.Errorf("error hashing choice data: %v, err: %v", choice, err))
			}
			c.hashChoices.Append(hash, choice)
		}
	}

	// Add choices to our array in non-hashable case
	if c.choices != nil {
		c.choices = append(c.choices, choices...)
	}
}

// RemoveByHash removes the first choice from the WeightedRandomChooser whose hash is equal to the provided hash, returns a boolean indicates if a choice was removed.
func (c *WeightedRandomChooser[T]) RemoveByHash(hash common.Hash) bool {
	// Acquire our lock during the duration of this method.
	c.randomProviderLock.Lock()
	defer c.randomProviderLock.Unlock()

	if c.hashChoices == nil {
		panic(fmt.Errorf("RemoveByHash is not supported for non-hashable types"))
	}

	if choice, exists := c.hashChoices.Get(hash); exists {
		c.totalWeight = new(big.Int).Sub(c.totalWeight, choice.weight)
		c.hashChoices.Remove(hash)
		return true
	}
...
func (c *WeightedRandomChooser[T]) Choose() (*T, error) {
	...
	var choices []*WeightedRandomChoice[T]
	// Obtain our choices from different sources, depending on if T is hashable or not.
	if c.hashChoices != nil {
		choices = c.hashChoices.Values()
	} else {
		choices = c.choices
	}

	// If we have no choices or 0 total weight, return nil.
	if len(choices) == 0 || c.totalWeight.Cmp(big.NewInt(0)) == 0 {
