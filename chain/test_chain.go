...
type TestChain struct {
	...
	IsOnChain         bool
	IsShrunking       bool
	CacheAccountState *state.AccountState
	// PreStateRecorder  StateRecorder
}
...
// for invariant
func (t *TestChain) CallContractOnChain(msg *core.Message, state *state.StateDB, additionalTracers ...vm.EVMLogger) (*core.ExecutionResult, error) {
	// If our provided state is nil, use our current chain state.
	if state == nil {
		state = t.state
	}
	...
	// Perform our state transition to obtain the result.
	res, err := core.NewStateTransition(evm, msg, gasPool).TransitionDbOnChain()

	// Revert to our state snapshot to undo any changes.
	state.RevertToSnapshot(snapshot)

	// for revert TraceAccountState
	t.state.AccountState().TraceAccountState.TouchedBalanceMap[msg.From] = t.state.GetBalance(msg.From)
...
func (t *TestChain) PendingBlockAddTx(message *core.Message) error {
	...
	if t.IsOnChain {
		message.SkipAccountChecks = true
		receipt, executionResult, err = vendored.EVMApplyTransactionOnChain(message, t.chainConfig, &t.pendingBlock.Header.Coinbase, gasPool, t.state, t.pendingBlock.Header.Number, t.pendingBlock.Hash, tx, &usedGas, evm)
	} else {
...
// PendingBlockCommit commits a pending block to the chain, so it is set as the new head. The pending block is set
// to nil after doing so. If there is no pending block when calling this function, an error is returned.
func (t *TestChain) PendingBlockFakeCommit() error {
	var err error
	// If we have no pending block, we cannot commit it.
	if t.pendingBlock == nil {
		return fmt.Errorf("could not commit chain's pending block, as no pending block was created")
...
func (t *TestChain) commitStateOnChain(block uint64) error {
	blockNumber := big.NewInt(int64(block))
	root, err := t.state.Commit(t.chainConfig.IsEIP158(blockNumber))
	if err != nil {
		return fmt.Errorf("test chain state write error: %v", err)
	}
	if err := t.state.Database().TrieDB().Commit(root, false); err != nil {
		// If we encountered an error, reset our state, as we couldn't add the tx.
		t.state, _ = state.New(t.Head().Header.Root, t.stateDatabase, nil)
		return fmt.Errorf("test chain trie write error: %v", err)
	}
	return nil
}

func (t *TestChain) SetInternalCallHook(ichook vm.InternalCallHook) {
	t.vmConfigExtensions.InternalCallHook = ichook
}
