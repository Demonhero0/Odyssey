...
func TestCorpusReadWrite(t *testing.T) {
	...
	testutils.ExecuteInDirectory(t, t.TempDir(), func() {
		...
		assert.NoError(t, err)
		assert.EqualValues(t, corpus.mutableSequenceFiles.files.Len(), len(matches))
...
func TestCorpusCallSequenceMarshaling(t *testing.T) {
	...
	testutils.ExecuteInDirectory(t, t.TempDir(), func() {
		...
		for _, entryFile := range corpus.mutableSequenceFiles.files.Values() {
			// Marshal the entry
		...
		// Remove all items
		for _, file := range corpus.mutableSequenceFiles.files.Values() {
			corpus.mutableSequenceFiles.removeFile(file.fileName)
		}
		assert.Empty(t, corpus.mutableSequenceFiles.files.Values())
	})
}
