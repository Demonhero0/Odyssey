...
type corpusDirectory[T any] struct {
	...
	files *container.HashLinkedList[string, *corpusFile[T]] // key: fileName in lowercase

	// filesToRemove represents the corpusFile items just removed from the files, but not yet deleted from disk. These corpusFile would be deleted from disk when writeFiles is called.
	filesToRemove *container.HashLinkedList[string, *corpusFile[T]]
...
func newCorpusDirectory[T any](path string) *corpusDirectory[T] {
	return &corpusDirectory[T]{
		path:          path,
		files:         container.NewHashLinkedList[string, *corpusFile[T]](),
		filesToRemove: container.NewHashLinkedList[string, *corpusFile[T]](),
	}
}

func (cd *corpusDirectory[T]) Len() int {
	return cd.files.Len()
}
...
func (cd *corpusDirectory[T]) addFile(fileName string, data T) error {
	...
	// If the file was removed but not yet deleted from disk, we remove it from the list of files to remove.
	lowerFileName := strings.ToLower(fileName)
	if _, exists := cd.filesToRemove.Get(lowerFileName); exists {
		cd.filesToRemove.Remove(lowerFileName)
	}

	// First we make sure this file doesn't already exist, if it does, we overwrite its data and mark it unwritten.
	if file, exists := cd.files.Get(lowerFileName); exists {
		file.data = data
		file.writtenToDisk = false
		return nil
	}

	// If the file otherwise did not exist, we add it.
	cd.files.Append(lowerFileName, &corpusFile[T]{
		fileName:      fileName,
		data:          data,
		writtenToDisk: false,
	})
...
func (cd *corpusDirectory[T]) removeFile(fileName string) bool {
	...
	// If we find the filename, remove it from our list of files， and add it to the list of files to remove.
	lowerFileName := strings.ToLower(fileName)
	if file, exists := cd.files.Get(lowerFileName); exists {
		cd.files.Remove(lowerFileName)
		cd.filesToRemove.Append(lowerFileName, file)
		return true
	}
	return false
}
...
func (cd *corpusDirectory[T]) readFiles(filePattern string) error {
	...
	cd.files = container.NewHashLinkedList[string, *corpusFile[T]]()
...
func (cd *corpusDirectory[T]) writeFiles() error {
	...
	filessToRemove := cd.filesToRemove
	cd.filesToRemove = container.NewHashLinkedList[string, *corpusFile[T]]()
	...
	// Remove files to remove from disk
	for _, file := range filessToRemove.Values() {
		filePath := filepath.Join(cd.path, file.fileName)

		if err := os.Remove(filePath); err != nil && !os.IsNotExist(err) {
			return fmt.Errorf("An error occurred while removing corpus file: %v\n", err)
		}
	}
	...
	for _, file := range cd.files.Values() {
