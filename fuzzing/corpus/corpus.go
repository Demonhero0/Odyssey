...
type Corpus struct {
	...
	// invariantMaps describes the total invariant distance known to be achieved across all corpus call sequences.
	invariantMaps *invariant.InvariantMaps

	// recording seed
	seedQueues map[string][]common.Hash
...
func NewCorpus(corpusDirectory string, fuzzingConfig *config.FuzzingConfig) (*Corpus, error) {
	...
	corpus := &Corpus{
		...
		storageWriteSet:         storagewrite.NewStorageWriteSet(),
		...
		// recording invariant
		invariantMaps: invariant.NewInvariantMaps(),
...
func (c *Corpus) CallSequenceEntryCount(mutable bool, immutable bool, testResults bool) int {
	count := 0
	if mutable {
		count += c.mutableSequenceFiles.files.Len()
	}
	if immutable {
		count += c.immutableSequenceFiles.files.Len()
	}
	if testResults {
		count += c.testResultSequenceFiles.files.Len()
	}
	return count
}
...
func (c *Corpus) initializeSequences(sequenceFiles *corpusDirectory[calls.CallSequence], testChain *chain.TestChain, deployedContracts map[common.Address]*contracts.Contract, useInMutations bool) error {
	...
	var err error
	for _, sequenceFileData := range sequenceFiles.files.Values() {
...
func (c *Corpus) Initialize(baseTestChain *chain.TestChain, contractDefinitions contracts.Contracts) (int, int, error) {
	...
	c.storageWriteSet = storagewrite.NewStorageWriteSet()
	storageWriteTracer := storagewrite.NewStorageWriteTracer()
	...
	testChain, err := baseTestChain.Clone(func(newChain *chain.TestChain) error {
		...
		if c.fuzzingConfig.UseStorageWriteTracing() {
			newChain.AddTracer(storageWriteTracer, true, false)
		}
...
func (c *Corpus) addCallSequence(sequenceFiles *corpusDirectory[calls.CallSequence], sequence calls.CallSequence, useInMutations bool, mutationChooserWeight *big.Int, flushImmediately bool) error {
	...
	//for _, existingSeq := range sequenceFiles.files.Values() {
	...
	//	}
	//}
	// As we use hash as file name, we can check this by checking if the file name exists.
...
// Returns an error if one occurs.
func (c *Corpus) CheckSequenceCoverageAndUpdate(callSequence calls.CallSequence, mutationChooserWeight *big.Int, flushImmediately bool, isAddCallSequence bool) error {
