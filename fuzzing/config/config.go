...
type FuzzingConfig struct {
	...
	// SenderBalances
	SenderAddressesBalances []*big.Int `json:"senderAddressesBalances"`
	...
	// InvariantEnabled describes whether to use invariant-guided fuzzing
	VariableRecoverConfig VariableRecoverConfig `json:"variableRecoverConfig"`

	// onchain-fuzzing config
	OnChainFuzzingConfig OnChainFuzzingConfig `json:"onChainFuzzingConfig"`

	// state guided enabled
	StateGuidedConfig StateGuidedConfig `json:"stateGuidedConfig"`

	// MetricRecordConfig describes the configuration used for metric record
...
type TestingConfig struct {
	...
	// InvariantTesting describes the configuration used for invariant testing.
	InvariantChecking InvariantCheckingConfig `json:"invariantChecking"`

	// ReentrancyEnable
	ContractCallConfig ContractCallConfig `json:"contractCallConfig"`
}
...
type VariableRecoverConfig struct {
	...
	SlitherScriptPath string `json:"slitherScriptPath"`
}

type OnChainFuzzingConfig struct {
	...
	CacheContractPath     string   `json:"cacheContractPath"`
	CacheAccountStatePath string   `json:"cacheAccountStatePath"`
...
type MetricLogConfig struct {
	...
	// TimeInterval describes the time interval in seconds for which the metric log should be outputted.
	TimeInterval int    `json:"time_interval"`
	TimeLogFile  string `json:"time_log_file"`

	// SequenceInterval describes the call sequence number interval for which the metric log should be outputted.
	SequenceInterval int    `json:"sequence_interval"`
	SequenceLogFile  string `json:"sequence_log_file"`
}

// InvariantTestingConfig describes the configuration options used for invariant testing
type InvariantCheckingConfig struct {
	// Enabled describes whether testing is enabled.
	Enabled         bool   `json:"enabled"`
	JsonPath        string `json:"jsonPath"`
	InvariantGuided bool   `json:"invariantGuided"`
	// InvariantList bool `json:"testViewMethods"`
}
...
func (f *FuzzingConfig) UseCoverageTracing() bool {
	return f.CoverageEnabled || f.MetricRecordConfig.CoverageEnabled
}

func (f *FuzzingConfig) UseBranchCoverageTracing() bool {
	return f.MetricRecordConfig.BranchCoverageEnabled
}

func (f *FuzzingConfig) UseStorageWriteTracing() bool {
	return f.MetricRecordConfig.StorageWriteEnabled
}

func (f *FuzzingConfig) UseStateTracing() bool {
	return f.StateGuidedConfig.EnabledStateGuided || f.MetricRecordConfig.StateEnabled
}

func (f *FuzzingConfig) UseHelperContract() bool {
	return f.Testing.ContractCallConfig.EnabledContractCall || f.Testing.ContractCallConfig.EnabledInternalCall
}

func (f *FuzzingConfig) UseStateGuided() bool {
	return f.StateGuidedConfig.EnabledStateGuided
}

func (f *FuzzingConfig) UseNewStateScope() bool {
	return f.StateGuidedConfig.EnabledNewScope
}
