...
// for invariant
func (cs CallSequence) AttachTxTraces(chain *chain.TestChain, contractDefinitions fuzzingTypes.Contracts) error {
	// For each call sequence element, attach an execution trace.
	for _, cse := range cs {
		err := cse.AttachTxTrace(chain, contractDefinitions)
		if err != nil {
			return err
		}
	}
...
func (cs CallSequence) Log() *logging.LogBuffer {
	...
	for i := 0; i < len(cs); i++ {
		...
		if cs[i].TxTrace != nil {
			buffer.Append(cs[i].TxTrace.Log().Elements()...)
			buffer.Append("\n")
		} else if cs[i].ExecutionTrace != nil {
...
type CallSequenceElement struct {
	...
	// for invariant
	TxTrace *invariant.ExecutionTrace `json:"-"`

	// for contract call
	UnableSendWithHelperContract bool                   `json:"unableSendWithHelperContract"`
	IsHelperContractCall         bool                   `json:"-"`
	OriginalContract             *fuzzingTypes.Contract `json:"-"`
	OriginalCall                 *CallMessage           `json:"originalCall"`
	// for internal call
	UnableInternalCall bool          `json:"-"`
	InternalCall       *InternalCall `json:"internalCall"`
}
...
func (cse *CallSequenceElement) Clone() (*CallSequenceElement, error) {
	...
	var clonedOriginalCall *CallMessage
	clonedOriginalCall = nil
	if cse.OriginalCall != nil {
		clonedOriginalCall, err = cse.OriginalCall.Clone()
		if err != nil {
			return nil, err
		}
	}
	...
	clone := &CallSequenceElement{
		...
		// for heplerContract
		IsHelperContractCall:         cse.IsHelperContractCall,
		OriginalContract:             cse.OriginalContract,
		OriginalCall:                 clonedOriginalCall,
		InternalCall:                 cse.InternalCall,
		UnableSendWithHelperContract: cse.UnableSendWithHelperContract,
	}
...
func (cse *CallSequenceElement) String() string {
	if cse.IsHelperContractCall {
		return cse.stringForHelperContractCall()
	}
...
func (cse *CallSequenceElement) stringForHelperContractCall() string {
	// Obtain our contract name
	contractName := "<unresolved contract>"
	if cse.OriginalContract != nil {
		contractName = cse.OriginalContract.Name()
	}

	// Obtain our method name
	var method *abi.Method
	// If we have a method resolved, return it.
	if cse.OriginalCall != nil && cse.OriginalCall.DataAbiValues != nil {
		method = cse.OriginalCall.DataAbiValues.Method
	}
	methodName := "<unresolved method>"
	if method != nil {
		methodName = method.Sig
	...
	// Next decode our arguments (we jump four bytes to skip the function selector)
	args, err := method.Inputs.Unpack(cse.OriginalCall.Data[4:])
	argsText := "<unable to unpack args>"
	if err == nil {
		argsText, err = valuegeneration.EncodeABIArgumentsToString(method.Inputs, args)
		if err != nil {
	...
	// Return a formatted string representing this element.
	return fmt.Sprintf(
		"ContractCall: %s.%s(%s) (block=%s, time=%s, gas=%d, gasprice=%s, value=%s, sender=%s)",
		contractName,
		methodName,
		argsText,
		blockNumberStr,
		blockTimeStr,
		cse.OriginalCall.GasLimit,
		cse.OriginalCall.GasPrice.String(),
		cse.OriginalCall.Value.String(),
		cse.OriginalCall.From,
	)
}
...
func (cse *CallSequenceElement) AttachExecutionTrace(chain *chain.TestChain, contractDefinitions fuzzingTypes.Contracts) error {
	...
	// deal with internal call
	if cse.InternalCall != nil {
		ichook := NewInterCallHookForReplay(cse.InternalCall)
		chain.SetInternalCallHook(ichook)
	}
...
func (cse *CallSequenceElement) AttachTxTrace(chain *chain.TestChain, contractDefinitions fuzzingTypes.Contracts) error {
	// Verify the element has been executed before.
	if cse.ChainReference == nil {
		return fmt.Errorf("failed to resolve execution trace as the chain reference is nil, indicating the call sequence element has never been executed")
	}
	...
	// Perfrom our call with the given trace
	_, cse.TxTrace, err = invariant.CallWithTxTrace(chain, contractDefinitions, cse.Call.ToCoreMessage(), state)
	if err != nil {
		return fmt.Errorf("failed to resolve execution trace due to error replaying the call: %v", err)
	}
	return nil
