...
func (g *CallSequenceGenerator) PopSequenceElement() (*calls.CallSequenceElement, error) {
	...
	// to solve insufficient funds
	element.Call.Value = new(big.Int).Mod(element.Call.Value, g.worker.Chain().State().GetBalance(element.Call.From))
	if element.Call.GasPrice.Cmp(big.NewInt(0)) == 1 {
		gas := new(big.Int).Mul(big.NewInt(int64(element.Call.GasLimit)), element.Call.GasPrice)
		element.Call.Value = new(big.Int).Sub(element.Call.Value, gas)
	}

	if element.Call.Value.Cmp(big.NewInt(0)) < 0 {
		element.Call.Value = big.NewInt(0)
	}

	// send with helperContract
	if g.worker.fuzzer.config.Fuzzing.Testing.ContractCallConfig.EnabledContractCall {
		if !element.UnableSendWithHelperContract && element.OriginalCall == nil && g.worker.randomProvider.Float32() > g.worker.fuzzer.config.Fuzzing.Testing.ContractCallConfig.ContractCallProbability {
			element, _ = ConvertToContractCall(element)
		}
	}

	// for skip account check
	element.Call.SkipAccountChecks = true
