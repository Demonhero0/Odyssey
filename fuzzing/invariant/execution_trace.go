...
func CallWithTxTrace(chain *chain.TestChain, contractDefinitions contracts.Contracts, msg *core.Message, state *state.StateDB) (*core.ExecutionResult, *ExecutionTrace, error) {
	// Create an execution tracer
	txTracer := NewTxTracer()
	txTracer.SetRecodingState([]common.Address{})
	txTracer.SetContracts(contractDefinitions, chain.CheatCodeContracts())

	// Call the contract on our chain with the provided state.
	executionResult, err := chain.CallContract(msg, state, txTracer)
	if err != nil {
		return nil, nil, err
	}

	// Obtain our trace
	trace := txTracer.GetTrace()
	// Return the trace
...
func (t *ExecutionTrace) generateElementsAndLogsForCallFrame(currentDepth int, callFrame *CallFrame) ([]any, []any) {
	...
	newElements, consoleLogString, callArgValueMap, inteinterestingValues := t.generateCallFrameEnterElements(callFrame)
	elements = append(elements, prefix)
	elements = append(elements, newElements...)

	callArgElements := t.logABIArgValueMap(callArgValueMap, currentDepth)
	elements = append(elements, callArgElements...)

	// Add the state change elements
	newElements = t.generateStateElements(callFrame, currentDepth, inteinterestingValues)
	elements = append(elements, newElements...)
	...
	if callFrame.IsContract {
		...
		for _, eventLog := range callFrame.Logs {
...
func (t *ExecutionTrace) generateCallFrameEnterElements(callFrame *CallFrame) ([]any, string, map[string]ArgValue, map[common.Hash][]*ArgValue) {
	...
	if callFrame.CodeContractAbi != nil {
		...
		if callFrame.IsContractCreation() {
			...
			method, err = callFrame.CodeContractAbi.MethodById(callFrame.Input)
			if err == nil {
	...
	if method != nil {
		...
		} else if len(callFrame.Input) >= 4 {
			abiDataInputBuffer = callFrame.Input[4:]
		}
		...
		if err == nil {
			...
			argValueMap, _ = TravelABIArgs(method.Inputs, inputValues, interestingValues)
			...
			if callFrame.To == chain.ConsoleLogContractAddress {
	...
	if inputArgumentsDisplayText == nil {
		temp := fmt.Sprintf("msg_data=%v", hex.EncodeToString(callFrame.Input))
	...
	if callFrame.IsProxyCall() {
		if callFrame.IsContract {
			callInfo = fmt.Sprintf("%v -> %v.%v(%v) (addr=%v, code=%v, value=%v, sender=%v)", proxyContractName, codeContractName, methodName, *inputArgumentsDisplayText, callFrame.To.String(), callFrame.CodeAddress.String(), callFrame.Value, callFrame.From.String())
		} else {
			callInfo = fmt.Sprintf("(addr=%v, value=%v, sender=%v)", callFrame.To.String(), callFrame.Value, callFrame.From.String())
		}
	} else {
		if callFrame.IsContract {
			if callFrame.To == chain.ConsoleLogContractAddress {
				...
				callInfo = fmt.Sprintf("%v.%v(%v) (addr=%v, value=%v, sender=%v)", codeContractName, methodName, *inputArgumentsDisplayText, callFrame.To.String(), callFrame.Value, callFrame.From.String())
			}
		} else {
			callInfo = fmt.Sprintf("(addr=%v, value=%v, sender=%v)", callFrame.To.String(), callFrame.Value, callFrame.From.String())
		}
	...
	return elements, consoleLogString, argValueMap, interestingValues
}

func (t *ExecutionTrace) logABIArgValueMap(argValueMap map[string]ArgValue, currentDepth int) []any {
	elements := make([]any, 0)
	if len(argValueMap) > 0 {
		prefix := strings.Repeat("\t", currentDepth+1) + " => "
		elements = append(elements, prefix, colors.Blue, "[input] ")

		k := 0
		for name, argValue := range argValueMap {
			stateDisplayText := fmt.Sprintf("%v: %v", name, argValue.Value)
			if k != 0 {
				elements = append(elements, strings.Repeat("\t", currentDepth+1)+"            ")
			}
			elements = append(elements, colors.Reset, stateDisplayText, "\n")
			k++
		}
	}
	return elements
...
func (t *ExecutionTrace) generateStateElements(callFrame *CallFrame, currentDepth int, interestingValues map[common.Hash][]*ArgValue) []any {
	// Create list of elements
	elements := make([]any, 0)

	if callFrame.PreState == nil || callFrame.PostState == nil {
		// elements = append(elements, colors.Reset, "no state \n")
		return elements
	}

	currentContract := callFrame.To

	// find interesting values
	interestingValues[common.BytesToHash(callFrame.From[:])] = append(interestingValues[common.BytesToHash(callFrame.From[:])], &ArgValue{
	...
	mergedStateVariables, varNameList := GetDynamicVariables(callFrame, interestingValues)
	if len(varNameList) > 0 {
		prefix := strings.Repeat("\t", currentDepth+1) + " => "
		elements = append(elements, prefix, colors.Yellow, "[state] ")
		k := 0
		for varName := range varNameList {
			preVarName := fmt.Sprintf("pre(%v)", varName)
			postVarName := fmt.Sprintf("post(%v)", varName)
			stateDisplayText := fmt.Sprintf("%v: %v => %v", varName, mergedStateVariables[preVarName].Value, mergedStateVariables[postVarName].Value)
			if k != 0 {
				elements = append(elements, strings.Repeat("\t", currentDepth+1)+"            ")
			}
			elements = append(elements, colors.Reset, stateDisplayText, "\n")
	...
	// elements = append(elements, strings.Repeat("\t", currentDepth+1)+"            ")
	// if _, ok := callFrame.PreState[currentContract]; ok {
	// 	k := 0
	// 	for slot := range callFrame.PreState[currentContract].Storage {
	// 		stateDisplayText := fmt.Sprintf("<unresolved> slot=[%v], [%v] => [%v]", slot, callFrame.PreState[currentContract].Storage[slot].String(), callFrame.PostState[currentContract].Storage[slot].String())
	// 		if k != 0 {
	// 			elements = append(elements, strings.Repeat("\t", currentDepth+1)+"            ")
	// 		}
	// 		elements = append(elements, colors.Reset, stateDisplayText, "\n")
...
func (t *ExecutionTrace) generateCallFrameExitElements(callFrame *CallFrame) []any {
	...
	if callFrame.CodeContractAbi != nil {
		if callFrame.IsContractCreation() {
			...
			method, _ = callFrame.CodeContractAbi.MethodById(callFrame.Input)
		}
	}
	...
	if method != nil {
		...
		if callFrame.Error == nil {
			outputValues, err := method.Outputs.Unpack(callFrame.Output)
			if err == nil {
	...
	if outputArgumentsDisplayText == nil && len(callFrame.Output) > 0 {
		temp := fmt.Sprintf("return_data=%v", hex.EncodeToString(callFrame.Output))
	...
	panicCode := abiutils.GetSolidityPanicCode(callFrame.Error, callFrame.Output, true)
	if panicCode != nil {
	...
	errorMessage := abiutils.GetSolidityRevertErrorString(callFrame.Error, callFrame.Output)
	if errorMessage != nil {
	...
	matchedCustomError, unpackedCustomErrorArgs := abiutils.GetSolidityCustomRevertError(callFrame.CodeContractAbi, callFrame.Error, callFrame.Output)
	...
	if callFrame.Error == vm.ErrExecutionReverted {
	...
	fmt.Println(callFrame.Error)
	elements = append(elements, colors.RedBold, fmt.Sprintf("[vm error ('%v')]", callFrame.Error.Error()), colors.Reset, "\n")
	return elements
}
...
func (t *ExecutionTrace) generateEventEmittedElements(callFrame *CallFrame, log *Log) []any {
	// Create list of elements
	elements := make([]any, 0)
