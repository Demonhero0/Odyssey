...
var precedence = map[string]int{"=": 10, "<": 10, ">": 10, "+": 20, "-": 20, "*": 40, "/": 40, "%": 40, "^": 60}
...
func (a *AST) parseNumber() NumberExprAST {
	...
	// i64, err := strconv.ParseInt(a.currTok.Tok, 10, 64)
	val, flag := new(big.Int).SetString(a.currTok.Tok, 10)
	if flag == false {
		a.Err = errors.New(
			fmt.Sprintf("error in numberExprAST. \nwant '(' or '0-9' but get '%s'\n%s",
				a.currTok.Tok,
				ErrPos(a.source, a.currTok.Offset)))
