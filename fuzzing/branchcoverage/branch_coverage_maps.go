...
func (cm *CoverageMaps) DumpCoverage(includeReverted bool) DumpCoverage {
	cm.lock.RLock()
	...
	for i := range cm.maps {
		...
		for j := range cm.maps[i] {
			c[i.String()][j.String()] = cm.maps[i][j].getCoverageByteMap(includeReverted)
		}
	}
	return c
}

func (cm *CoverageMaps) TotalBranchCoverage(includeReverted bool, targetAddresses []common.Address) (int, int) {
	cm.lock.RLock()
	defer cm.lock.RUnlock()
	...
	for i := range cm.maps {
		if len(targetAddresses) > 0 {
			...
			for _, j := range cm.maps[i] {
				c, t := j.getCoverageRate(includeReverted)
				coveredBranchSize += c
				totalBranchSize += t
			}
		}
...
type ContractCoverageMap struct {
	...
	revertedCoverage *CoverageMapBranchData
}
...
func (cm *ContractCoverageMap) getCoverageRate(includeReverted bool) (int, int) {
	if !includeReverted {
		return cm.successfulCoverage.getCoverageRate()
	}
	allCoverage := &CoverageMapBranchData{}
	_, _ = allCoverage.update(cm.successfulCoverage)
	_, _ = allCoverage.update(cm.revertedCoverage)
	return allCoverage.getCoverageRate()
}

func (cm *ContractCoverageMap) getCoverageByteMap(includeReverted bool) []byte {
	if !includeReverted {
	...
	_, _ = allCoverage.update(cm.successfulCoverage)
	_, _ = allCoverage.update(cm.revertedCoverage)
	return allCoverage.executedFlags
}
