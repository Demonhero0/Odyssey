...
func (t *CoverageTracer) CaptureEnd(output []byte, gasUsed uint64, err error) {
	...
	if err != nil {
		...
		if revertCoverageErr != nil {
			logging.GlobalLogger.Panic("Block coverage tracer failed to update revert coverage map during capture end", revertCoverageErr)
		}
	}
...
func (t *CoverageTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
	...
	if err != nil {
		_, revertCoverageErr := t.callFrameStates[t.callDepth].pendingCoverageMap.RevertAll()
		if revertCoverageErr != nil {
			logging.GlobalLogger.Panic("Block coverage tracer failed to update revert coverage map during capture exit", revertCoverageErr)
		}
	}
	...
	if coverageUpdateErr != nil {
		logging.GlobalLogger.Panic("Block coverage tracer failed to update coverage map during capture exit", coverageUpdateErr)
	}
...
func (t *CoverageTracer) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, vmErr error) {
	...
	if len(scope.Contract.Code) > 0 && op == vm.JUMPI {
		...
		cond := !scope.Stack.Back(1).IsZero()
		...
		_, coverageUpdateErr := callFrameState.pendingCoverageMap.SetAt(scope.Contract.Address(), *callFrameState.lookupHash, branchSize, branchId)
		if coverageUpdateErr != nil {
