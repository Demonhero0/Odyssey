...
type FuzzerHooks struct {
	...
	//for invariant
	GetInvariantPreStateFunc  func(worker *FuzzerWorker, callSequenceElement *calls.CallSequenceElement) error
	GetInvariantPostStateFunc func(worker *FuzzerWorker, callSequence calls.CallSequence) error
	InvariantCheckFuncs       []InvariantCheckFunc
}
...
type InvariantCheckFunc func(worker *FuzzerWorker, callSequence calls.CallSequence, lowestDistance *big.Int) ([]ShrinkCallSequenceRequest, error)
...
type ShrinkCallSequenceRequest struct {
	...
	// is shrinking inptu values
	IsShrinkInputValues bool
}
