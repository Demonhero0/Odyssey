...
func encodeABIArgumentToString(inputType *abi.Type, value any) (string, error) {
	...
	case abi.AddressTy:
		...
		if addr == common.HexToAddress(helperContract) {
			return strings.ToLower(addr.String()) + "[HelperContract]", nil
		} else if addr == common.HexToAddress(sender) {
			return strings.ToLower(addr.String()) + "[Sender]", nil
		} else {
