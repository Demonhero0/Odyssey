...
type Fuzzer struct {
	...
	fuzzerInitAccountState  FuzzerInitAccountState
	proxyContractMap        map[common.Address]common.Address
	targetContractAddresses []common.Address
...
type StateAndCoverage struct {
	TimeSecond  string                `json:"time"`
	DumpState   invariant.DumpState   `json:"dumpState"`
	DumpSlot    invariant.DumpSlot    `json:"dumpSlot"`
	DumpCoverge coverage.DumpCoverage `json:"dumpCoverage"`
}
...
func NewFuzzer(config config.ProjectConfig) (*Fuzzer, error) {
	...
	if fuzzer.config.Compilation != nil {
		...
		if fuzzer.config.Fuzzing.VariableRecoverConfig.TraceStorage {
			err = invariant.InitStorageExtractors(compilations, "crytic-export", fuzzer.config.Fuzzing.TargetContracts, fuzzer.config.Fuzzing.VariableRecoverConfig.SlitherScriptPath, "")
			if err != nil {
				fuzzer.logger.Error("Failed to extract storage layout", err)
				return nil, err
			}
		}
	}
	...
	for err == nil && working {
		...
		go func(workerSlotInfo availableWorkerSlot) {
			...
			if err == nil {
				...
				var workerErr error
				// if f.config.Fuzzing.OnChainFuzzingConfig.IsOnChain {
				// 	ctxCancelled, workerErr = worker.runOnChain(baseTestChain)
				// } else {
...
func (f *Fuzzer) printMetricsLoop() {
	...
	for !utils.CheckContextDone(f.ctx) {
		...
		// for on-chain fuzzing
		if f.config.Fuzzing.OnChainFuzzingConfig.IsOnChain {
			logBuffer.Append(", rpc-count: ", colors.Bold, fmt.Sprintf("%d", rpc.Provider.Count), colors.Reset)
		}
