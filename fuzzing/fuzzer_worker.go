...
type FuzzerWorker struct {
	...
	// invariantTracer describes the tracer used to collect invariant maps during fuzzing campaigns.
	txTracer *invariant.TxTracer

	// for on-chain fuzzing
	proxyContractMap   map[common.Address]common.Address
	contractViewMethod map[common.Address]map[string]abi.Method
...
func newFuzzerWorker(fuzzer *Fuzzer, workerIndex int, randomProvider *rand.Rand) (*FuzzerWorker, error) {
	...
	// to deal with proxy contract
	worker.proxyContractMap = make(map[common.Address]common.Address)
	for proxy, implementation := range fuzzer.proxyContractMap {
		worker.proxyContractMap[proxy] = implementation
	}
...
func (fw *FuzzerWorker) updateStateChangingMethods() {
	...
	for contractAddress, contractDefinition := range fw.deployedContracts {
		if contractAddress == FuzzHelperContractAddr {
			continue
		}

		// If we deployed the contract, also enumerate property tests and state changing methods.
		fw.contractViewMethod[contractAddress] = make(map[string]abi.Method)
		...
		for _, method := range contractDefinition.CompiledContract().Abi.Methods {
			if !method.IsConstant() {
				...
				fw.contractViewMethod[contractAddress][method.Sig] = method
			}
		}
		...
		if implementation, ok := fw.proxyContractMap[contractAddress]; ok {
			bytecodes, _ := fw.chain.CacheAccountState.GetCode(implementation)

			// Try to match it to a known contract definition
			matchedDefinition := fw.fuzzer.contractDefinitions.MatchBytecode(nil, bytecodes)
			// If we didn't match any deployment, report it.
			if matchedDefinition != nil {
				for _, method := range matchedDefinition.CompiledContract().Abi.Methods {
					if !method.IsConstant() {
						// Any non-constant method should be tracked as a state changing method.
						fw.stateChangingMethods = append(fw.stateChangingMethods, fuzzerTypes.DeployedContractMethod{Address: contractAddress, Contract: matchedDefinition, Method: method})
					} else {
						fw.contractViewMethod[contractAddress][method.Sig] = method
					}
				}
...
func (fw *FuzzerWorker) testNextCallSequence() (calls.CallSequence, []ShrinkCallSequenceRequest, error) {
	...
	if fw.chain.IsOnChain {
		fw.chain.State().SetAccountState(fw.chain.CacheAccountState)
		fw.chain.State().SyncInitAccountStateToStateDB(fw.chain.CacheAccountState)
		fw.chain.PendingBlockCreate()
		fw.chain.PendingBlockFakeCommit()
		defer func() {
			fw.chain.CacheAccountState.RecoverToInitAccountState()
		}()
	}
	...
	fetchElementFunc := func(currentIndex int) (*calls.CallSequenceElement, error) {
		callSequenceElement, err := fw.sequenceGenerator.PopSequenceElement()

		if callSequenceElement != nil {
			if fw.fuzzer.config.Fuzzing.Testing.InvariantChecking.Enabled {
				err = fw.fuzzer.Hooks.GetInvariantPreStateFunc(fw, callSequenceElement)
				if err != nil {
					return nil, err
				}
			}

			// init InternalCallHoos
			if fw.fuzzer.config.Fuzzing.Testing.ContractCallConfig.EnabledInternalCall {
				ichook = NewInternalCallHook(fw.fuzzer.config.Fuzzing.Testing.ContractCallConfig.InternalCallProbability, fw.sequenceGenerator, callSequenceElement)
				fw.chain.SetInternalCallHook(ichook)
			}
		}

		return callSequenceElement, err
	}

	lowestDistance := big.NewInt(-2)

	// old state variables
	var oldStateVariables map[string]*invariant.StateValue
	oldStateVariables = nil
	...
	// request.
	executionPostCheckFunc := func(currentlyExecutedSequence calls.CallSequence) (bool, error) {
		...
		err := fw.fuzzer.corpus.CheckSequenceMetricAndUpdate(currentlyExecutedSequence, fw.getNewCorpusCallSequenceWeight(), true, fw.fuzzer.config.Fuzzing.CoverageEnabled)
		if err != nil {
			return true, err
		}

		fw.fuzzer.corpus.RemoveTracerResults(currentlyExecutedSequence)

		if fw.fuzzer.config.Fuzzing.UseStateTracing() {
			stateVariables, err := fw.getStateVariables(currentlyExecutedSequence)
			if err != nil {
				return true, err
			}

			if stateVariables != nil {
				// add interesting value to seed pool
				fw.updateSeedPool(stateVariables)

				err = fw.fuzzer.corpus.CheckSequenceStateCoverageAndUpdate(
					currentlyExecutedSequence,
					fw.getNewCorpusCallSequenceWeight(),
					true,
					fw.fuzzer.config.Fuzzing.UseStateGuided(),
					oldStateVariables,
					stateVariables,
				)
				if err != nil {
				...
				oldStateVariables = make(map[string]*invariant.StateValue)
				for name, item := range stateVariables {
					oldStateVariables[name] = item
				}
			}
		}
		...
		// checking is new and interesting state
		// if fw.fuzzer.config.Fuzzing.UseStateTracing() {
		// 	newShrinkRequests, err := fw.genShrinkRequestWithStateVariables(currentlyExecutedSequence)
		// 	if err != nil {
		// 		return true, err
		// 	}
		// 	shrinkCallSequenceRequests = append(shrinkCallSequenceRequests, newShrinkRequests...)
		...
		// checking invariant is satisfied
		if fw.fuzzer.config.Fuzzing.Testing.InvariantChecking.Enabled {
			// get state for invariant check
			err := fw.fuzzer.Hooks.GetInvariantPostStateFunc(fw, currentlyExecutedSequence)
			if err != nil {
				return true, err
			}
			// Loop through each invariant check function, signal our worker tested a call, and collect any requests to shrink
			// this call sequence.
			for _, invariantCheckFunc := range fw.fuzzer.Hooks.InvariantCheckFuncs {
				newShrinkRequests, err := invariantCheckFunc(fw, currentlyExecutedSequence, lowestDistance)
				if err != nil {
					return true, err
				}
				shrinkCallSequenceRequests = append(shrinkCallSequenceRequests, newShrinkRequests...)
	...
	testedCallSequence, err := calls.ExecuteCallSequenceIteratively(fw.chain, fetchElementFunc, executionPostCheckFunc)

	// fmt.Println("lowestDistance:", lowestDistance, ":")
...
func (fw *FuzzerWorker) testShrunkenCallSequence(possibleShrunkSequence calls.CallSequence, shrinkRequest ShrinkCallSequenceRequest) (bool, error) {
	...
	defer func() {
		if err == nil {
			err = fw.chain.RevertToBlockNumber(fw.testingBaseBlockNumber)
			fw.chain.IsShrunking = false
		}
	}()

	if fw.chain.IsOnChain {
		fw.chain.IsShrunking = true
		fw.chain.State().SetAccountState(fw.chain.CacheAccountState)
		fw.chain.State().SyncInitAccountStateToStateDB(fw.chain.CacheAccountState)
		fw.chain.PendingBlockCreate()
		fw.chain.PendingBlockFakeCommit()
	...
	fetchElementFunc := func(currentIndex int) (*calls.CallSequenceElement, error) {
		...
		if fw.fuzzer.config.Fuzzing.Testing.InvariantChecking.Enabled {
			err = fw.fuzzer.Hooks.GetInvariantPreStateFunc(fw, possibleShrunkSequence[currentIndex])
			if err != nil {
				return nil, err
			}
		}
		...
		// init InternalCallHoos
		if fw.fuzzer.config.Fuzzing.Testing.ContractCallConfig.EnabledInternalCall && possibleShrunkSequence[currentIndex].InternalCall != nil {
			ichook := calls.NewInterCallHookForReplay(possibleShrunkSequence[currentIndex].InternalCall)
			fw.chain.SetInternalCallHook(ichook)
		}
	...
	executionCheckFunc := func(currentlyExecutedSequence calls.CallSequence) (bool, error) {
		...
		seqErr := fw.fuzzer.corpus.CheckSequenceMetricAndUpdate(currentlyExecutedSequence, fw.getNewCorpusCallSequenceWeight(), true, fw.fuzzer.config.Fuzzing.CoverageEnabled)
		if seqErr != nil {
			return true, seqErr
		}
...
func (fw *FuzzerWorker) shrinkCallSequence(callSequence calls.CallSequence, shrinkRequest ShrinkCallSequenceRequest) (calls.CallSequence, error) {
	...
	if shrinkLimit > 0 {
		...
		for removalStrategy := 0; removalStrategy < 1 && !shrinkingEnded(); removalStrategy++ {
		...
		if shrinkRequest.IsShrinkInputValues {
			for !shrinkingEnded() {
				for i := len(optimizedSequence) - 1; i >= 0 && !shrinkingEnded(); i-- {
	...
	// fmt.Print("optimizedSequence", optimizedSequence)

	executionCheckFuncForFinishCallback := func(currentlyExecutedSequence calls.CallSequence) (bool, error) {
	...
	fetchElementFunc := func(currentIndex int) (*calls.CallSequenceElement, error) {
		if currentIndex < len(optimizedSequence) {
			if fw.fuzzer.config.Fuzzing.Testing.InvariantChecking.Enabled {
				err := fw.fuzzer.Hooks.GetInvariantPreStateFunc(fw, optimizedSequence[currentIndex])
				if err != nil {
					return nil, err
				}
			}
			...
			// init InternalCallHoos
			if optimizedSequence[currentIndex].InternalCall != nil {
				ichook := calls.NewInterCallHookForReplay(optimizedSequence[currentIndex].InternalCall)
				fw.chain.SetInternalCallHook(ichook)
			}
			return optimizedSequence[currentIndex], nil
		}
		return nil, nil
	}
	_, err := calls.ExecuteCallSequenceIteratively(fw.chain, fetchElementFunc, executionCheckFuncForFinishCallback)
	if err != nil {
		return nil, err
	}
...
func (fw *FuzzerWorker) run(baseTestChain *chain.TestChain) (bool, error) {
	...
	err = fw.beforeRun()
	if err != nil {
		return false, fmt.Errorf("error in beforeRun %v", err)
	}
	...
	for sequencesTested <= fw.fuzzer.config.Fuzzing.WorkerResetLimit {
		...
		for _, shrinkVerifier := range shrinkVerifiers {
			// for motivation
			// fmt.Println("time to catch bug:", time.Since(fw.fuzzer.startTime).Seconds(), ":")
